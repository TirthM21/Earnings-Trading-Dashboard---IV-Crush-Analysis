...
class EarningsTradingDashboard:
    def __init__(self, root):
        self.root = root
        self.root.title("Earnings Trading Dashboard - IV Crush Analysis (yfinance)")
    ...
    def setup_ui(self):
        ...
        self.scrollable_frame = ttk.Frame(self.canvas_scroll, padding="10")
        ...
        earnings_frame.grid(row=0, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        self.analyze_btn = ttk.Button(earnings_frame, text="Analyze IV Crush", command=self.analyze_iv_crush, state="normal")
        ...
        option_frame = ttk.LabelFrame(main_frame, text="ATM Straddle Pricing & Analysis", padding="5")
        ...
        # Advanced Straddle Stats
        ttk.Label(option_frame, text="Implied Move:").grid(row=3, column=0, padx=(0, 5))
        self.implied_move_label = ttk.Label(option_frame, text="N/A", font=("Arial", 11, "bold"), foreground="purple")
        self.implied_move_label.grid(row=3, column=1, padx=(0, 20))
        
        ttk.Label(option_frame, text="Breakevens:").grid(row=3, column=2, padx=(0, 5))
        self.breakeven_label = ttk.Label(option_frame, text="N/A", font=("Arial", 11))
        self.breakeven_label.grid(row=3, column=3, columnspan=3, padx=(0, 20), sticky=tk.W)
        
        # Add P/L analysis
        ttk.Label(option_frame, text="LONG Straddle P/L:").grid(row=4, column=0, padx=(0, 5))
        self.long_pnl_label = ttk.Label(option_frame, text="N/A", font=("Arial", 11, "bold"))
        self.long_pnl_label.grid(row=4, column=1, columnspan=2, padx=(0, 20))
        
        ttk.Label(option_frame, text="SHORT Straddle P/L:").grid(row=4, column=3, padx=(0, 5))
        self.short_pnl_label = ttk.Label(option_frame, text="N/A", font=("Arial", 11, "bold"))
        self.short_pnl_label.grid(row=4, column=4, columnspan=2)
        ...
        # Create matplotlib figure with 3 subplots for advanced analysis
        self.fig, (self.ax1, self.ax2, self.ax3) = plt.subplots(3, 1, figsize=(14, 18))
    ...
    def analyze_iv_crush(self):
        ...
        # Calculate date range (20 days buffer)
        start_date = self.earnings_date - timedelta(days=20)
        end_date = self.earnings_date + timedelta(days=20)
        
        # Query stock price data
        self.log_message(f"Downloading stock data for {self.ticker} via yfinance...")
        ...
        try:
            ...
            if stock_data.empty:
                self.log_message("No stock data found.")
                return
                
            # Normalize columns to lowercase
            # Handle MultiIndex if present (yfinance sometimes returns (Price, Ticker))
            ...
            self.log_message(f"Error downloading stock data: {e}")
            messagebox.showerror("Error", f"Failed to download stock data: {e}")
            return
            
        # Query VIX data
        self.log_message("Downloading VIX data...")
        try:
            ...
            self.log_message(f"Error downloading VIX data: {e}")
            self.vix_data = None
        
        # Query IV data for the stock - Not directly available in yfinance history
        self.log_message(f"Note: Historical implied volatility not available via yfinance. Using VIX estimation.")
        self.iv_data = None
    ...
    def perform_iv_crush_analysis(self):
        ...
        # Market Implied Move ~ 85% of Straddle Price (typical rule of thumb) or just the straddle price
        implied_move = pre_straddle_price
        implied_move_pct = (implied_move / pre_stock_price) * 100
        
        upper_be = atm_strike_price + pre_straddle_price
        lower_be = atm_strike_price - pre_straddle_price
        
        self.implied_move_label.config(text=f"±${implied_move:.2f} (±{implied_move_pct:.1f}%)")
        self.breakeven_label.config(text=f"Lower: ${lower_be:.2f} | Upper: ${upper_be:.2f}")
    ...
    def create_visualizations(self):
        ...
        # Plot 1: Stock price and IV around earnings
        self.ax1.plot(self.stock_data.index, self.stock_data['close'], 'b-', linewidth=2, label='Stock Price')
        self.ax1.axvline(x=self.earnings_date, color='red', linestyle='--', alpha=0.7, label='Earnings Date')
        self.ax1.set_xlabel('Date')
        self.ax1.set_ylabel('Stock Price ($)', color='blue')
        self.ax1.tick_params(axis='y', labelcolor='blue')
        ...
        for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
            ...
            self.ax2.text(bar1.get_x() + bar1.get_width()/2., height1 + 0.1,
                        f'${height1:.1f}', ha='center', va='bottom', fontsize=9)
            self.ax2.text(bar2.get_x() + bar2.get_width()/2., height2 + 0.1,
        ...
        # Plot 3: VIX or P/L distribution
        if self.vix_data is not None:
             self.ax3.plot(self.vix_data.index, self.vix_data['close'], 'purple', linewidth=2, label='VIX')
             self.ax3.axvline(x=self.earnings_date, color='red', linestyle='--', alpha=0.7, label='Earnings Date')
             self.ax3.set_xlabel('Date')
             self.ax3.set_ylabel('VIX Level')
             self.ax3.set_title('Market Volatility (VIX) Context')
             self.ax3.grid(True, alpha=0.3)
             self.ax3.legend()
        else:
             self.ax3.text(0.5, 0.5, "VIX Data Unavailable", ha='center', va='center')
